...
Production Order lifecycle mutations use explicit CQRS commands. Create and generic update accept planning data only; new rows start Planned, and conditional persistence prevents an update/delete racing with Release. Release locks the current active BOM without reserving inventory. Start recalculates locked-BOM requirements server-side, validates explicit tenant-owned warehouse allocations, and delegates one whole-order transaction to `IProductionExecutionRepository`. A conditional Released-to-InProgress update prevents double-start; conditional balance decrements, `ProductionConsume` ledger inserts, and `StartedAt` commit together. Completion, output inventory, and InProgress cancellation remain deferred.
