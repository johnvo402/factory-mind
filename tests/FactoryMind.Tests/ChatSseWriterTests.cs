...
public sealed class ChatSseWriterTests {
    ...
    public async Task Stream_writes_conversation_tokens_citations_and_done_in_order() {
        ...
        await writer.WriteAsync(
            context,
            new ChatStream(conversationId, Updates(citation)),
    ...
    private static async IAsyncEnumerable<ChatStreamUpdate> Updates(CitationResponse citation) {
        await Task.Yield();
        yield return new ChatTokenUpdate("Stop");
        yield return new ChatTokenUpdate(" now [S1].");
